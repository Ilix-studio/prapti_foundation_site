...
const EditorAddPhoto: React.FC = () => {
  ...
  const handleFileSelect = useCallback(
    (files: FileList) => {
      ...
      if (uploadMode === "single" && incoming.length > 1) {
      ...
      setSelectedFiles((prev) => {
        ...
        if (next.length > MAX_FILES) {
          ...
          // Revoke the overflow previews we just created.
          next.slice(MAX_FILES).forEach((f) => URL.revokeObjectURL(f.preview));
          return next.slice(0, MAX_FILES);
        }
        // If replacing in single mode, revoke the previous preview.
  ...
  if (!isAuthenticated || (!isEditor && !isAdmin)) {
    return (
      <div className='container mx-auto p-6'>
        <BackNavigation />
        <div className='max-w-md mx-auto mt-8 flex items-center gap-2 rounded-lg border p-4'>
          <AlertCircle className='h-4 w-4' />
          <span>Editor access required.</span>
        </div>
      </div>
    );
  }
